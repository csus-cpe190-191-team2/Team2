...
def thresholdingY(img):
    imgHsv = cv2.cvtColor(img, cv2.COLOR_BGR2HSV)
    lowerYellow = np.array([0,36,175])    # Green: 27, 68, 178
    upperYellow = np.array([104,255,255])   # Green: 35, 112, 223
...
def get_histogramY(img, min_per=0.1, display=False, region=1):
...
def get_lane_curve(img, display=0):
    ...
    # ## Step 3
    mid_point, img_hist = get_histogramY(img_warp, display=True, min_per=0.5, region=4)
    curve_avg_point, img_hist = get_histogramY(img_warp, display=True, min_per=0.9)
    ...
    if display != 0:
        ...
        cv2.putText(img_result, str(curve/10), (w_t // 2 - 80, 85), cv2.FONT_HERSHEY_COMPLEX, 2, (255, 0, 255), 3)
...
if __name__ == '__main__':
    while cap.isOpened():
        ...
        # gray = cv2.cvtColor(img, cv2.COLOR_BGR2HSV)
        img_thresh = thresholding(img)
        initialize_trackbars([50, 132, 0, 187])

        h_t, w_t, ch = img.shape
        width_top = 30       # 67
        height_top = 137     # 112
        width_bottom = 0    # 0
        height_bottom = 240  # 180
        points = np.float32([(width_top, height_top), (w_t - width_top, height_top),
                             (width_bottom, height_bottom), (w_t - width_bottom, height_bottom)])
        img_thresh_warp = warp_image(img_thresh, points, w_t, h_t)
        img_warp = warp_image(img, points, w_t, h_t)
