...
class LaneFinder:
    ...
    def draw_triangle(self, dx, y, inv=1):
        out_img = self.img.copy()
        img_shape = out_img.shape
        y = y % 240
        dx = dx % (480 / 2)
        cv2.line(out_img, (int(img_shape[1] / 2), 0), (int(img_shape[1] / 2), int(y)),
                 (255, 255, 0), 2) #vertical line
        cv2.line(out_img, (int(img_shape[1] / 2), int(y)), (int((img_shape[1] / 2) + (dx * inv)), int(y)),
                 (255, 255, 0), 2) #horizontal line
        cv2.line(out_img, (int(img_shape[1] / 2), 0), (int((img_shape[1] / 2) + (dx * inv)), int(y)),
                 (255, 255, 0), 2) #hypotenuse
        hor_diff = dx * inv
        ver_diff = y  # will always be positive
        self.angle = math.degrees(math.atan2(hor_diff, ver_diff))
        print('Angle: {0:.2f} degrees'.format(self.angle))
        cv2.imshow('Triangle IMG',out_img)
        if cv2.waitKey(1) & 0xFF == ord('q'):
            self.error = 1
            self.destroy()

    def thresholding(self):
        print(self.points[0]['thresh_points'])
        thresh_pts = self.points[0]['thresh_points']
        img_copy = self.img.copy()
        img_hsv = cv2.cvtColor(img_copy, cv2.COLOR_BGR2HSV)
        lower_thresh = np.array([38,0,14])
        upper_thresh = np.array([179,255,213])
        mask = cv2.inRange(img_hsv, lower_thresh, upper_thresh)
        self.img_thresh = mask
        while True:
            cv2.imshow('IMG', self.img_thresh)
            if cv2.waitKey(1) & 0xFF == ord('q'):
                break
    ...
    def sliding_window(self, nwindows=9, margin=60, minpix=10, draw_windows=False):  # polyfit and window drawing and finds angle
        ...
        for window in range(nwindows):
            ...
            if draw_windows:
                ...
                cv2.line(self.out_img, (int(img_shape[1] / 2), int(img_shape[0])), (int(img_shape[1] / 2), 0),
                         (255, 255, 0), 2)  # middle of the img
        ...
        # cv2.line(self.out_img, (upper_point[0],upper_point[1]), (lower_point[0],lower_point[1]), (0,0,255), 3)
        cv2.line(self.out_img, (top_second[0], top_second[1]), (bottom_one[0], bottom_one[1]), (79, 252, 17),
                 2)  # short line
    ...
    def get_histogram(self, display=False, region=1):
        ...
        if display:
            ...
            for x, intensity in enumerate(hist_vals):
                cv2.line(img_hist, (x, self.img_warp.shape[0]),
                         (x, self.img_warp.shape[0] - int(intensity) // 255 // region),
                         (255, 0, 255), 1)
    ...
    ######
    def display(self, left_cr, right_cr):
        self.draw_lanes(left_cr, right_cr)
    ...
    def draw_lanes(self, left_curve, right_curve):
        ...
        cv2.putText(inv_perspective, 'Lane Curve: {:}'.format(text_str1),
                    (inv_perspective.shape[1] // 2 - 220, inv_perspective.shape[0] - 215), font, font_size, font_color,
                    2)
        cv2.putText(inv_perspective, 'Lane Radii: {:}'.format(text_str2),
                    (inv_perspective.shape[1] // 2 - 220, inv_perspective.shape[0] - 190), font, font_size, font_color,
                    2)
        cv2.putText(inv_perspective, 'Angle: {:.4f}'.format(self.angle),
                    (inv_perspective.shape[1] // 2 - 220, inv_perspective.shape[0] - 165), font, font_size, font_color,
                    2)
